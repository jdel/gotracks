...
```bash
cp .env.example .env
# set GOTRACKS_AUTH_JWT_SECRET — generate with: openssl rand -hex 32
docker compose up -d
```

Open <http://localhost:8080> and register. **The first account to register
becomes the administrator** after accepting its emailed invitation — so on a
publicly reachable instance, register it before exposing the port.
